...
public class DestructibleObject : MonoBehaviour
{
    ...
    private new Rigidbody rigidbody;
        ...
        private float collisionForceMagnitude; // 衝突の勢いを保存
        ...
        void OnCollisionEnter(Collision collision)
        {
            if (collision.relativeVelocity.magnitude > forceRequired)
            {
                ...
                collisionForceMagnitude = collision.relativeVelocity.magnitude; // 衝突の勢いを取得
        ...
        for(int i = 0; i < debris.transform.childCount; i++){
            Rigidbody debrisRigidbody = debris.transform.GetChild(i).GetComponent<Rigidbody>();
                ...
                // 衝突の法線方向に衝突の勢いを加算し、ランダムな要素を追加
                Vector3 forceDirection = collisionNormal * collisionForceMagnitude * Random.Range(0.1f, 0.3f);
                Vector3 randomForce = Random.insideUnitSphere * (collisionForceMagnitude * 0.3f); // ランダムな方向に少しばらつかせる

                debrisRigidbody.AddForce(forceDirection + randomForce, ForceMode.Impulse); // 力を適用
